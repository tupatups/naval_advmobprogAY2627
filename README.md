...
- Lab Activity 4: In this activity, I implemented a layered architecture using the service pattern to separate my UI from the data-fetching logic. When a user logs in, my service layer executes an API request, converts the JSON response into a user model, and saves the data locally via SharedPreferences. To render the profile screen, I use this locally stored user ID to fetch the complete user details from the API. This updated design keeps my code modular, decoupled, and type-safe. Finally, to render the cart screen, I asynchronously retrieve this saved user ID and pass it to a cart-specific API endpoint, ensuring only the logged-in user's items are displayed.  